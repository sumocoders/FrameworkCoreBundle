...
Requested as a port of an existing consuming project's `App\EventListener\SentryUserContextListener`
(fetched for reference via the GitLab API) into `FrameworkCoreBundle`, generalized for use across all consuming
projects and gated behind bundle config.
...
2. **Sentry is an optional dependency.** Do not add `sentry/sentry-symfony` to composer `require`. Add it to
   ...
   the reference implementation itself uses) so the bundle installs and works fine in projects that never touch
   Sentry.
3. **Config: single on/off flag**, default `true`. Something like:
   ...
   lands.
4. **Impersonation tracking is in scope.** Port the reference implementation's approach: detect via
   ...
   `impersonation` ⇒ `['impersonator_id' => ...]`), mirroring the shape of the reference implementation's
   `impersonator_id` / `impersonator_email` pair minus the email field (see decision 1 — no email anywhere in
   this feature).
